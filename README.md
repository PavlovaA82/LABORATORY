...
Далее вводим команду: 
 ...
 ----------------------------------------------------------------------------------------------------------------------------------------
 
Подразумевается, что у нас установлен Docker и docker-compose. Создаем каталоги, где будем создавать наши файлы:

 ➤ `sudo mkdir -p /opt/prometheus_stack/{prometheus,grafana,alertmanager,blackbox}`- успешное создание директорий с правами root.

 ➤ `sudo touch /opt/prometheus_stack/docker-compose.yml`- создание файла docker-compose.yml.

 ➤ `cd /opt/prometheus_stack`-  переход в новую директорию.

 ![image](https://github.com/user-attachments/assets/5c18ec4f-13ac-44c1-96d1-95de40466903)
...
![image](https://github.com/user-attachments/assets/2ce29df1-aaed-40cd-befa-54aa41e5758b)

Открываем файл и вставляем `node-exporter`:

 ➤ `sudo vi docker-compose.yml`

 ![image](https://github.com/user-attachments/assets/fa068eed-2d4a-4e96-8189-cab0421a29d3)

Далее прописываем в браузере linux: 
  * `localhost:9100` 
 
И должно перевести на эту страницу:

![image](https://github.com/user-attachments/assets/24e18484-9dd6-4e96-b453-e266559fc891)
...
![image](https://github.com/user-attachments/assets/0290b240-1402-4cf8-a6d2-66eaa32aa9fe)
 ...
 Настроим отправку уведомлений на телеграм бот. Само оповещение будет выполнять AlertManager, который интегрируется с `Prometheus`. Для начала подготовим наш бот телеграм. Создадим нового — для этого открываем телеграм и ищем `@my_id_bot`:

 ![image](https://github.com/user-attachments/assets/77b4c62f-4396-49e4-acdd-6290dc45ae05)

Приложение отобразит наш идентификатор. Необходимо зафиксировать данный идентификатор, так как он потребуется в дальнейшем.Далее следует создать бота. Для этого необходимо осуществить поиск официального аккаунта `@BotFather`. Процесс создания бота осуществляется посредством последовательного ввода команды `/newbot` и последующего предоставления необходимых данных в ответ на запросы программного интерфейса мастера.

![image](https://github.com/user-attachments/assets/f52d5e20-c77c-43da-bd05-8d842aa72117)

Запускаем бота:

![image](https://github.com/user-attachments/assets/5739320c-c4e8-492f-be10-b3fe70068939)

Открываем наш файл docker-compose: 

 ➤ `sudo vi docker-compose.yml` 

И добавляем: alertmanager-bot с этой ссылки - https://github.com/PavlovaA82/LABORATORY/blob/main/2/docker-compose.yml
